...
def get_branches_list():
    """جلب كل الفروع كأسماء فقط مع ID لاستخدام الأزرار"""
    branches = get_branches()
    return [(b_id, b_name) for b_id, b_name in branches]
...
def add_department(branch_id: int, name: str):
    ...
    with get_conn() as conn:
        with conn.cursor() as cur:
            cur.execute(
                "INSERT INTO departments (branch_id, name) VALUES (%s, %s)",
                (branch_id, name)
            )
        conn.commit()
...
def get_departments(branch_id: int):
    """جلب الأقسام لفرع محدد"""
    with get_conn() as conn:
        with conn.cursor() as cur:
            cur.execute(
                "SELECT id, name FROM departments WHERE branch_id = %s ORDER BY name",
                (branch_id,)
            )
            return cur.fetchall()


def get_departments_list(branch_id: int):
    """جلب قائمة أسماء الأقسام مع ID لاستخدام الأزرار"""
    departments = get_departments(branch_id)
    return [(d_id, d_name) for d_id, d_name in departments]
...
def add_contact(department_id: int, name: str, phone: str):
    """إضافة اسم ورقم لقسم محدد"""
    with get_conn() as conn:
        with conn.cursor() as cur:
            cur.execute(
                "INSERT INTO contacts (department_id, name, phone) VALUES (%s, %s, %s)",
...
def get_contacts(department_id: int):
    """جلب كل الأسماء والأرقام لقسم محدد"""
    with get_conn() as conn:
        with conn.cursor() as cur:
            cur.execute(
                "SELECT id, name, phone FROM contacts WHERE department_id = %s ORDER BY name",
                (department_id,)
            )
            return cur.fetchall()


def get_contacts_list(department_id: int):
    """جلب قائمة الأسماء مع الأرقام لاستخدام الأزرار"""
    contacts = get_contacts(department_id)
    return [(c_id, c_name, c_phone) for c_id, c_name, c_phone in contacts]
