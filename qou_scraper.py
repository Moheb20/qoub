...
class QOUScraper:
    ...
    def fetch_balance_table(self) -> str:
        """
        يرجع رصيد الطالب على شكل جدول أنيق وثابت الأعمدة مناسب للهواتف على Telegram
        """
        ...
        # رأس الجدول
        header = "📊 *رصيد الطالب*\n\n"
        columns = ["📅 الفصل", "💰 المطلوب", "💸 المدفوع", "🔙 السابق", "🎁 المنح", "🧾 رصيد الفصل", "💱 العملة"]
    
        # نص الجدول مع أعمدة ثابتة
        table_text = ""
        # عرض العناوين
        for col in columns:
            table_text += f"{col:<12} "
        ...
        for row in rows:
            ...
            if len(cols) < 7:
                continue
            # كل قيمة في العمود تأخذ نفس العرض
            table_text += f"{cols[0]:<12} {cols[1]:<12} {cols[2]:<12} {cols[3]:<12} {cols[4]:<12} {cols[5]:<12} {cols[6]:<6}\n"
